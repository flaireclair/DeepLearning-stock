...
from get_first_working_day import retrun_first_day_of_working as fwd
...
for b_num in brand_nikkei :
    while index != []:
        try :
            if is_already_index :
                df_stock.append(pd.read_csv("../stock_data/201{0}_tousyou1bu_all/only_closing_stock_data_with_date/stock_data_with_date_{1}.csv".format(str(index[0]), b_num)))
            else :
                df_stock = pd.read_csv("../stock_data/201{0}_tousyou1bu_all/only_closing_stock_data_with_date/stock_data_with_date_{1}.csv".format(str(index[0]), b_num))
